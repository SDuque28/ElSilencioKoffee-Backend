...
| Dashboard | `features/dashboard/services/dashboard.service.ts` | `getMetrics()` used by `dashboard-home-page.component.ts` | `/dashboard/metrics` via `apiUrl` | `GET` | None | `DashboardMetric[]` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | KPI dashboard has no backend implementation. | Implement dashboard metrics endpoint and align metric shapes before removing mocks. |
| Dashboard | `features/dashboard/services/dashboard.service.ts` | `getSalesMetrics(startDate, endDate)` used by home and sales dashboard pages | `/dashboard/sales?startDate=&endDate=` via `apiUrl` | `GET` | Query params `startDate`, `endDate` | `SalesMetric[]` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | Sales charts depend entirely on mocks. | Implement backend sales aggregation endpoint with date filters. |
| Dashboard | `features/dashboard/services/dashboard.service.ts` | `getTopBuyers()` used by `dashboard-users-page.component.ts` | `/dashboard/top-buyers` via `apiUrl` | `GET` | None | `TopBuyer[]` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | Top-buyer ranking has no backend implementation. | Implement backend customer ranking endpoint and document pagination/sorting behavior if needed. |
| Production | `features/production/services/production.service.ts` | `listProduction(page, limit)` used by `production-page.component.ts` | `/production?page=&limit=` via `apiUrl` | `GET` | Query params `page`, `limit` | `Production[]` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | Production analytics UI has no matching backend controller despite SQL tables for production. | Implement production CRUD/listing endpoints aligned with the existing schema. |
| Environment | `features/environment-monitoring/services/environment-monitoring.service.ts` | `listReadings(page, limit)` used by `environment-monitoring-page.component.ts` | `/environment/readings?page=&limit=` via `apiUrl` | `GET` | Query params `page`, `limit` | `EnvironmentReading[]` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | Historical environment charts have no backend implementation. | Implement environment readings endpoints and pagination contract. |
| Environment | `features/environment-monitoring/services/environment-monitoring.service.ts` | `getLatestReading()` currently defined but not used by page logic | `/environment/readings/latest` via `apiUrl` | `GET` | None | `EnvironmentReading` | None found | None | None | None | None | Frontend admin-only route | FRONTEND ONLY | Latest-reading contract exists in frontend service only. | Implement the endpoint or remove the unused service method until the backend exists. |
